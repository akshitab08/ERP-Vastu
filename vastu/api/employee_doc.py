...
def organize_employee_documents(doc, method):
    ...
    drive_employee_folder = create_drive_folder(employee_code, base_drive_folder_id, google_drive)
    ...
    for i in doc.get("custom_document_", []):
        ...
        drive_doc_folder = create_drive_folder(i.document_type, drive_employee_folder, google_drive)

    
        file_id = upload_file_to_drive(drive_doc_folder, google_drive)
...
def create_drive_folder(folder_name, parent_folder_id, google_drive):
...
def upload_file_to_drive(target_drive_folder_id, google_drive):

   
    last_file_doc = frappe.get_last_doc("File")
    file_url = last_file_doc.file_url

    if not file_url:
        frappe.logger().error("No file URL found for the last uploaded file.")
        return None


    if file_url.startswith("/files/"):
        file_path = os.path.join(get_bench_path(), "sites", frappe.local.site, "public", file_url.lstrip("/"))
    else:
